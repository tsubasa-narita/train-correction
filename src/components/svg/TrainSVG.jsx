...
const TrainSVG = ({ train: t, parts, animating, animProgress, stepCount }) => {
  const id = t.id;
  ...
  return (
    <svg viewBox="0 0 480 275" style={{ width: "100%", maxWidth: 480, height: "auto", display: "block", margin: "0 auto", overflow: "visible" }}>
      ...
      {/* 台車・スカート (ボディ依存) */}
      <g style={{ opacity: hb ? 1 : 0.12, ...(hb ? bS : {}) }}><rect x={bx + 8} y={bb} width={bw - 16} height={12} rx={2} fill={hb ? t.under : "#ddd"} /><rect x={bx + 4} y={bb + 8} width={bw - 8} height={8} rx={2} fill={hb ? t.skirt : "#e8e8e8"} /></g>
      {/* ボディ本体 */}
      ...
      <g style={{ opacity: showRoof ? 1 : 0.12, ...(showRoof ? roofS : {}) }}>
        {hb
          ? <g clipPath={`url(#bc-${id})`}><rect x={bx - 5} y={by - 2} width={bw + tipX - bx + 15} height={14} rx={5} fill={showRoof ? t.roof : "#ddd"} /></g>
          : <rect x={bx} y={by} width={bw} height={14} rx={isSteam ? 4 : 8} fill={showRoof ? t.roof : "#ddd"} />}
      </g>
      {/* まど */}
      <g style={{ opacity: hwin ? 1 : 0.12, ...(hwin ? winS : {}) }}>
        {wins.map((w, i) => (<g key={i}><rect x={w.x} y={w.y} width={w.w} height={w.h} rx={3} fill={hwin ? t.winBg : "#ddd"} /><rect x={w.x + 1.5} y={w.y + 1.5} width={w.w - 3} height={w.h - 3} rx={2} fill={hwin ? t.window : "#e5e5e5"} /></g>))}
      </g>
      {/* ドア (独立描画) */}
      ...
      <g style={{ opacity: hw ? 1 : 0.12, ...(hw ? wS : {}) }}>
        {isSteam
          ? [150, 180, 210, 268, 288].map((cx, i) => <circle key={i} cx={cx} cy={i < 3 ? 248 : 250} r={i < 3 ? 12 : 7} fill={hw ? (i < 3 ? "#8B0000" : "#4A4A4A") : "#d0d0d0"} />)
          : [[bx + 30, bx + 56], [bx + bw - 56, bx + bw - 30]].map((pair, gi) => (
            <g key={gi}>
              {hw && <rect x={pair[0] - 10} y={bb + 12} width={pair[1] - pair[0] + 20} height={7} rx={2} fill={t.bogie} />}
